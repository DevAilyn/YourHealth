...
const Menu = () => {
  ...
  return (
    <nav className='flex items-center justify-between px-6 py-3 bg-white rounded-full shadow-lg max-w-6xl mx-auto'>
      {/* Logo */}
      <img className='w-40 h-8 cursor-pointer' src={assets.logo} alt='Logo'  />
...

         {
            ...
            ? <div className='flex items-center gap-5 cursor-pointer group relative'>
                ...
                <div className='absolute top-0 right-0 w-26 pt-18 text-base font-medium text-black z-20 hidden group-hover:block'>
                    <div className='bg-darkblue-bg rounded-2xl w-35 h-30 flex flex-col justify-center pl-3 gap-1'>
                        <p onClick={()=>navigate('/MiPerfil')} className='hover:text-amber-200 cursor-pointer'>Mi perfil</p>
                        <p onClick={()=>navigate('/MisReservas')} className='hover:text-amber-200 cursor-pointer'>Mis reservas</p>
                        <p onClick={()=>setToken(false)} className='hover:text-amber-200 cursor-pointer'>Cerrar Sesión</p>
                    </div>
                </div>
            </div>
